...
| FR-004 | Duplicate flight number + time rejected | `service/FlightService.java` | `FlightService.createFlight()` duplicate check | None | Confirmed |
| FR-005 | New flight ETD = STD, delay = 0 | `service/FlightService.java` | `FlightService.createFlight()` set fields | TS-001 | Confirmed |
| FR-006 | UUID auto-assigned on creation | `repository/FlightRepository.java` | `FlightRepository.save()` — `UUID.randomUUID()` | TS-001 | Confirmed |
...
| FR-011 | New ETD must not be >5 min in past | `service/FlightService.java` | `FlightService.updateEtd()` validation block | None | Confirmed |
| FR-012 | Status auto-recomputed after ETD update | `service/FlightService.java`, `service/EtdCalculationService.java` | `FlightService.updateEtd()` → `computeStatus()` | TS-002 | Confirmed |
...
| FR-015 | Estimated additional delay per reason | `service/EtdCalculationService.java` | `EtdCalculationService.estimateAdditionalDelay()` | None | Confirmed |
| FR-016 | `applyDelay()` bumps ETD, sets status=DELAYED | `service/EtdCalculationService.java` | `EtdCalculationService.applyDelay()` | None (used in seed) | Confirmed |
| FR-017 | Status lifecycle: SCHEDULED→BOARDING→DEPARTED | `service/EtdCalculationService.java` | `EtdCalculationService.computeStatus()` | None | Confirmed |
| FR-018 | Delayed flight shows DELAYED not BOARDING | `service/EtdCalculationService.java` | `EtdCalculationService.computeStatus()` | None | Confirmed |
| FR-019 | Terminal states not recomputed | `service/EtdCalculationService.java` | `EtdCalculationService.computeStatus()` — entry guard | None | Confirmed |
| FR-020 | Mark departed + record ATD | `service/FlightService.java` | `FlightService.markDeparted()` | None | Confirmed |
| FR-021 | Cancel flight with optional note | `service/FlightService.java` | `FlightService.cancelFlight()` | None | Confirmed |
...
| FR-026 | Status filter + sort controls | `static/js/dashboard.js` | `Dashboard.render()` — statusFilter, sortBy logic | None | Confirmed |
...
|---|---|---|---|---|
| NFR-001 | Thread-safe data store | `repository/FlightRepository.java` | `ConcurrentHashMap<String, Flight>` at class level | Confirmed |
...
| NFR-006 | No authentication/authorisation | `pom.xml` | No `spring-boot-starter-security` dependency | Confirmed |
| NFR-007 | Non-persistent in-memory store | `repository/FlightRepository.java` | `private final Map<String, Flight> flights = new ConcurrentHashMap<>()` | Confirmed |
...
| `WebConfig.java` | Not analysed | CORS or web configuration not examined in detail. |
...
**Test coverage: 5/28 FRs have any test coverage (18%)**  
**Confirmed** — single test class with 2 test methods.
